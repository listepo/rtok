...
There is no Homebrew formula for rtok, and `brew install rtok` is not a thing. Use the shell
installer above or `ketch install listepo/rtok`; both read the Release archives directly and need
no formula. The dist settings that would build one are commented out in `dist-workspace.toml`.

**To be fixed later.** The whole procedure is kept below as a comment rather than deleted, so
turning it on is uncommenting three lines. It is blocked on one secret and a decision, not on code.

<!-- HOMEBREW RELEASE — COMMENTED OUT, TO BE FIXED LATER
     Blocked on HOMEBREW_TAP_TOKEN, which only the repository owner can create. Do not uncomment
     any of this before that secret exists: `publish-homebrew-formula` runs AFTER the GitHub
     Release is created, so a missing token ends every release red having already published.

     The tap already exists — listepo/homebrew-tap, carrying Casks/ketch.rb.
     ...
     1. Add a HOMEBREW_TAP_TOKEN secret to this repository: a PAT that can push to
        ...
        listepo/ketch already has one, and a secret's value cannot be read back out of GitHub —
        issue a new PAT rather than trying to copy that one.
     2. Uncomment the three lines in dist-workspace.toml:
          installers = ["shell", "homebrew"]
          tap = "listepo/homebrew-tap"
          publish-jobs = ["homebrew"]
        and delete the plain `installers = ["shell"]` above them.
