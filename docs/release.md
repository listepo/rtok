...
`sha256` sums. No Homebrew formula: the tap has no `HOMEBREW_TAP_TOKEN` secret, so
`publish-homebrew-formula` would fail after the Release is created — install via the shell
installer above or `ketch install listepo/rtok`. `.github/workflows/release.yml` is generated by
`dist generate` — **never hand-edit it**; change `dist-workspace.toml` and run `just dist-generate`.
