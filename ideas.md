...
| I-92 | Creator request 2026-09-22; `research.md` §15.1 (WebFetch enters context whole, D2) | MCP (`fetch` tool) | WebFetch as an MCP replacement: a `fetch` tool that downloads a URL, extracts readable text (HTML→text, JSON minify), archives the raw bytes and returns the shortened text with an `expand <id>` pointer. Hooks cannot shrink WebFetch (PostToolUse adds context only, D2), the proxy live zone needs a base-URL host — an MCP tool is the surface that reaches every host. | Research only: step 1 measures the WebFetch/WebSearch share (`rtok stats` over real transcripts, WebSearch was 0.70 M est. tokens ≈ 5.4 % of tool-result tokens in the 2026-09-03 baseline); step 2 designs the tool (fetch limits, HTML→text rules, archive + `expand` contract, D21 singleton shape); promote to `plan.md` only if the share clears the 1 % gate and the design keeps lossless + `Measurement` rows. |
| I-97 | T180, `research.md` §20 (web results 9.6 % of tool-result bytes; 12 % of WebSearch links cited) | `cmd` / hooks / `proxy` | Web result formatter: `WebSearch` keeps the prose and only the links it cites, one `- title <url>` line each, the full `Links` array archived behind `expand <id>` (est. −42.8 % of WebSearch bytes); `WebFetch` head/tail above a byte cap, archived (est. −15.9 % at 8 KB). One pure function, wired behind the PostToolUse surface T134 confirms and into `proxy_filter` for proxy users. Byte-stable per `tool_use_id`. | **gate: T134** — the creator runs without the proxy, so only `updatedToolOutput` reaches their sessions; estimates are scratch numbers, not `Measurement` rows |
...
| I-93 | Code read 2026-09-23 (rayon / async-std review) | `measure` (`stats`) | `rtok stats` parses every session `.jsonl` sequentially (`src/measure/stats.rs` `collect` → `src/measure/codex.rs` `jsonl_paths`), folding into one report. Files are independent and no store writer sits in the loop, so a map-then-reduce with `rayon` (`par_iter().map(parse_file).reduce(merge)`) is the textbook shape. `rayon` is already in the workspace `rust.md`; adding it needs a `toolchain.md` row and a reason in the commit. | Needs a Check: wall time of `rtok stats` before/after on a real `~/.codex`/Claude session directory (≥ 1 000 files), recorded as a `Measurement` row. Without that number the saving does not exist. Only place where a new dependency pays for itself: the graph indexer (`each_parsed`) is already one worker per core (127 ms / 3 000 files, `research.md`), and the hook path (p95 11.7 ms vs 10 ms budget) cannot afford thread spawn. |
| I-94 | Code read 2026-09-23 | `doctor` | `rtok doctor` probes each configured MCP server one after another (`src/doctor.rs` `list_tools` per server), so the worst case is N × `mcp_timeout_ms`. Spawn the probes on a `std::thread::scope` and join; IO wait, not CPU, so no `rayon`. | Needs a Check: `doctor` wall time with ≥ 3 servers, one of them unreachable, before/after. Zero new dependencies. |
...
| I-95 | Code read 2026-09-23 | `read` (`search`, `tree`) | Both walk with `ignore::WalkBuilder::build()` (`src/plugins/read/search.rs`), single-threaded. `ignore` already ships `build_parallel()` (ripgrep's pool): `tree` maps directly (stat in parallel, sort once); `search` needs an atomic hit counter to keep the early exit at `cap`. Installed dependency, so it beats adding `rayon` here. | Needs a Check: p50/p95 of `search` and `tree` on this repo and on a 3 000-file tree before/after; output must stay byte-identical (sorted). Parallel walk may lose on small trees, so the Check decides, not the idea. |
...
| I-96 | Code read 2026-09-23 | `proxy` / `dashboard` | Blocking SQLite (Diesel) calls run inline on tokio workers: the dashboard `socket_loop` (`src/web/mod.rs`) opens the store and runs the snapshot queries every 2 s tick, and the proxy `handle` does request bookkeeping in the async hot path (`src/proxy/mod.rs`). Wrap them in `tokio::task::spawn_blocking` (already in the tree; no `spawn_blocking` exists in the repo today). Not a case for `async-std`: it is deprecated (2025, in favour of `smol`) and has the same blocking-executor hazard; all async here stays on tokio, and `rtok mcp` stays synchronous by design. | Needs a Check: proxy p95 latency under concurrent requests with the dashboard open, before/after; plus a test that a slow store does not stall other proxy requests. Related to the plan note on `snapshot()` loading every `Measurement` row per tick. |
...
| I-88 | T121 (`plugins/codex/`), Codex hooks doc (https://learn.chatgpt.com/docs/hooks) | hosts (`codex`) | Codex `PreToolUse` (Bash `updatedInput.command`) and `PostToolUse` (`additionalContext`) in both the installer and the plugin — the marketplace-install half shipped as T140 (`rtok agents install codex`, `codex plugin marketplace add`/`add`, D21 singleton). | Needs one live Bash PreToolUse payload through `rtok hook` before the tool hooks. |
| I-98 | `research.md` §23 (T262.2), Copilot hooks reference (https://docs.github.com/en/copilot/reference/hooks-reference) | hosts (`copilot`) | Copilot CLI `subagentStop` returns `modifiedResponse`, which replaces the subagent's answer before the parent reads it: a place to shorten long subagent reports losslessly (full text behind `expand <id>`). Needs a `Measurement` of subagent response sizes first. |
| I-99 | T156, `research.md` §18.4 (second data point, 2026-09-25) | `worktree` / `justfile` (`dunnage`, T236) | A reflink-seeded `target/` (`cp -c -R`, worktrunk `--copy-ignored`) skips every dependency rebuild, but `just test`'s `dunnage` pass then compresses the cloned files, costing ≈ 215 s and un-sharing ≈ 3.4 GiB; seeded `just check` measured 371 s against 185 s cold. Seeding pays only if that pass leaves clone-shared files alone, or if it runs on the source `target/` before cloning. | Neither variant is measured; adopting `reflink-copy` is a creator decision on those numbers (T156). |
| I-100 | Provider batch / flex tiers (Anthropic Message Batches, OpenAI Batch and Flex); `next.md` draft 2026-09-24 | `proxy` | Send traffic that can wait through the provider's batch or flex tier, which bills below the synchronous price and can stack with prompt caching where the provider allows it. rtok today proxies only synchronous agent turns. | No rtok traffic class is asynchronous yet; no `Measurement` row exists for any saving. |
| I-101 | Model routers (small model first, large only on hard turns); `next.md` draft 2026-09-24 | `proxy` | Route classify, lookup and simple-edit turns to a small model and send only hard turns to the large one. | It changes answer quality, not only tokens: needs a bench with a quality gate before any claim. |
