...
| I-91 | Agent SDK hooks page: `PostToolUse` `updatedToolOutput` "works for any tool" (`research.md` §17.2) | hooks / `archive` / `compress` | If the CLI honours it for command hooks, native Read/Bash output could be shrunk in place (pointer + `expand <id>`) instead of wrapped or denied. | Unverified for CLI command hooks; contradicts the standing rule "PostToolUse can only add context" — needs a live hook probe and a creator decision before any design. |
