...
| I-71 | tinyjuice (openhuman `TokenJuice`): HTML/RSS → readable text, 77.0 % smaller before any cache-and-recover, self-reported on a 15.4 MB / 166-case corpus (https://github.com/tinyhumansai/tinyjuice, read 2026-09-18) | `cmd` | **promoted T71.1** — The `[curl]` rule (`rules/default.toml`) cuts by position (head 10 / tail 10, keep `error|HTTP|curl:`), so an HTML page fetched with `curl`/`wget` keeps 20 lines of `<head>` boilerplate and drops the body text. A formatter for the curl family that detects `<html` in the first KB, drops `script`/`style`/tags and keeps text (raw page archived, `expand <id>`) is one Rust formatter, not a TOML rule, so it is outside T50.1. tinyjuice's other passes are already scheduled: JSON minify → T65.2, repeated lines `[×N]` → T64.2 / T65.1, skeletons → `read` map/signatures, originals behind a token → `archive`. | No measured share of HTML in the curl family's bytes; T50.1 step 1 (`stats` per family) gives that number. Add only if HTML is a visible slice there. |
...
| I-71 | T71.1 | `curl` / `wget` HTML pages as readable text (tinyjuice), gated on the T50.1 family share. | 2026-09-18 |
