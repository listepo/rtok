...
| I-88 | T121 (`plugins/codex/`), Codex hooks doc (https://learn.chatgpt.com/docs/hooks) | hosts (`codex`) | `rtok agents install codex --yes` offers `plugins/codex` (`codex plugin marketplace add <resolved path>`) and, while it is installed, strips its own `hooks.json` entries and `[mcp_servers.rtok]` (D21 singleton, the T115 shape); then Codex `PreToolUse` (Bash `updatedInput.command`) and `PostToolUse` (`additionalContext`) in both the installer and the plugin. | Needs a live Codex (the agent's shell allowlist blocks `codex`): plugin load, `marketplace add` with a `./` entry, and one Bash PreToolUse payload through `rtok hook`. |
