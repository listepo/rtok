...
| I-100 | Provider batch / flex tiers (Anthropic Message Batches, OpenAI Batch and Flex) | `proxy` | Send traffic that can wait through the provider's batch or flex tier, which bills below the synchronous price and can stack with prompt caching where the provider allows it. rtok today proxies only synchronous agent turns. | No rtok traffic class is asynchronous yet; no `Measurement` row exists for any saving. |
| I-101 | Model routers (small model first, large only on hard turns) | `proxy` | Route classify, lookup and simple-edit turns to a small model and send only hard turns to the large one. | It changes answer quality, not only tokens: needs a bench with a quality gate before any claim. |
