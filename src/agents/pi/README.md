...
`rtok archive rewrite --stdin`, T70.2) — both `cli` surface. When `[setup.pi] tools = true`,
`session_start` registers the measured MCP set through `pi.registerTool` as `rtok mcp --call`
(T70.3; 138 description tokens: read 17, search 12, tree 12, symbol 30, callers 27, expand 22,
mem_search 11, mem_get 7). Hook and proxy surfaces have no path in. `toon` declares MCP but is
not registered.
