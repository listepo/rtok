...
impl Agent for Cline {
    ...
    fn files(&self, cfg: &Config, _kind: Kind) -> Vec<PathBuf> {
        vec![
            cfg.setup.cline.hooks_path.clone(),
            cfg.setup.cline.mcp_path.clone(),
            ext_mcp_path(cfg),
        ]
    }
...
/// Drop `mcpServers.rtok` from a `cline_mcp_settings.json`, keeping foreign servers.
pub fn unregister_mcp(cfg: &Config, path: &Path) -> Result<String> {
    rtok_agent_sdk::unregister_server(&apply(cfg), path, "mcpServers", NAME)
}
