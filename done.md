...
Check result (2026-09-22): the 4 previously-failing tests (`log::tests::rotation_keeps_files_plus_the_current_one_and_the_newest_line_is_in_it`, `log::tests::a_rotation_storm_between_polls_prints_every_line_once`, `log::tests::a_rotation_decided_on_a_stale_size_is_dropped_under_the_lock`, `demon::tests::a_service_that_writes_past_max_bytes_gets_a_rotated_log`) pass locally on macOS; `just check` green; PR #189's `windows` CI job confirmed the fix on real Windows.

### T118.1. Hook adapter for Gemini CLI's event names and I/O shape

Split off T118 (`--host gemini` is a free string on `Config.hook.host`, not validated against a registry, so the hook adapter is testable without the host module, the plugin tree, or `HOSTS` registration — those move to T118.2/T118.3).

Do (2026-09-22): verified event names and JSON shapes against the Gemini CLI docs first (`docs/hooks/reference.md` and `docs/hooks/writing-hooks.md` on `google-gemini/gemini-cli` `main`, fetched 2026-09-22 — the current names are `BeforeTool`/`AfterTool`/`BeforeAgent`/`AfterAgent`/`BeforeModel`/`BeforeToolSelection`/`AfterModel`/`SessionStart`/`SessionEnd`/`Notification`/`PreCompress`, not the older `PreToolUse`/`PostToolUse` an extensions blog post still shows). Gemini's base envelope already uses Claude's own field names (`session_id`, `cwd`, `tool_name`, `tool_input`, `transcript_path`, `prompt`, `trigger`), so `HookInput::adapt_gemini` (`src/hooks/types.rs`) only maps event names (`BeforeTool`→`PreToolUse`, `AfterTool`→`PostToolUse`, `BeforeAgent`→`UserPromptSubmit`, `PreCompress`→`PreCompact` — same `auto`/`manual` trigger values; `SessionStart`/`SessionEnd` pass through; the remaining five events have no rtok plugin hook and fall through `dispatch`'s `_` arm) and aliases `tool_response.llmContent` to `stdout` so the shell/read plugins that key off Claude's flat shape still see the text. `gemini_output` (`src/hooks/mod.rs`) shapes the reply the other way: a top-level `{decision: "deny", reason}` to block (Gemini nests nothing under `permissionDecision`), `{hookSpecificOutput: {tool_input}}` to rewrite a call, `{hookSpecificOutput: {additionalContext}}` after one — mirroring the existing `copilot_output`/`cursor_output` per-host shaping functions. Wired into `dispatch_owned_strict` behind `cfg.hook.host == "gemini"`, same `!grok &&` guard the other hosts use.

Check result (2026-09-22): `cargo test --lib hooks::` — 29 passed (new: `gemini_before_after_tool_map_and_alias_llm_content_to_stdout`, `gemini_before_agent_and_pre_compress_map_to_claude_names`, `gemini_output_shapes_deny_rewrite_context_and_empty`), 0 failed; `just check` green.

Status: done 2026-09-22
Model: Claude Code / sonnet-5

### T130.1. Spawn brief: `SubagentStart` mechanism, config, and the shared digest builder
...
Check result (2026-09-22): `tests/copilot_plugin.rs` — manifest/component-path parity, hooks doc equality with `hooks_doc("rtok", 5)`, exact `.mcp.json`, and the fake-`copilot` e2e (dry-run names the command and runs nothing, `--yes` installs and strips the own-files pair, a repeat is `already installed` with no second CLI call, remove uninstalls); units for the offer/marker decision and the D21 singleton; `just check` green.

### T100. `rtok agents install grok` and the Read path under Grok

Do (2026-09-22): a `grok` host in `src/agents/grok/` (Kimi's "the host's own installer owns the state" shape, T86). Detection: `grok` on PATH plus `~/.grok/bin/grok` / `$GROK_HOME/bin/grok`. `support("plugin")` is an `Offer("--yes")` whose line prints `grok plugin install <resolved plugins/grok> --trust`; `installed()` reports `plugin` from the `~/.grok/plugins/rtok` marker (the copy `grok plugin install --trust` writes — `grok plugin list --json` reports the same state, the marker answers without spawning). While the plugin is absent, `[mcp_servers.rtok]` lands in `~/.grok/config.toml` via `toml_edit` (`[setup.grok] config_path`). D21 singleton, both halves: the plugin is the unit (its hooks + `.mcp.json` — an earlier `[mcp_servers.rtok]` is taken back, never added beside it), and while Grok's `[compat.claude]` import (default on) already fires rtok's Claude hooks/MCP — the FILES `~/.claude/settings.json` / `~/.claude.json`, never Claude's plugin (`claude::files_serve_rtok`, extracted so both sides read one helper) — setup says so instead of adding a second set. `docs/agents.md` blessed; `src/agents/grok/README.md` with `## Docs`.

Check result (2026-09-22): units for offer (the command line + ketch), detection (marker, table, the import) and both singletons; the `agents_install` matrix green with the grok row (install twice / remove twice / list); `agents_doc` + `host_docs` green; `just check` green. Open: the live `read_file` PreToolUse dump — `read_file` stays unmapped until a live payload confirms its `toolInput` matches what the Read plugins rewrite (then map `read_file` → `Read` and add the Read matcher to `plugins/grok`).
...
**T141 `rtok agents install|uninstall` closes a running host app before writing its config and reopens it after** · `src/agents/{mod.rs,restart.rs}`, `src/cli.rs`, `src/config/mod.rs`, `config/default.toml`, `docs/config.md`, `tests/{agents_install.rs,common/agents.rs,config_coverage.rs,cursor_plugin.rs,pi_plugin.rs}`, `tests/trycmd/*`
Do: replace T76's interactive y/N restart prompt with an automatic, no-prompt flow. For each host in a `rtok agents install|uninstall` request, only for a `Kind::Desktop` variant: if the app is running (checked without a launch side effect) and the write would actually change something for that host (dry-run via `agents::would_change`, reusing `apply_all` exactly as `run` does) — quit it before writing, write, then reopen it after. Not running, or nothing to change, does nothing to the process. CLI-only hosts are never killed; a running CLI binary just gets a one-line reminder to restart its session. New `Procs` trait (`running`/`quit`/`open`) with `RealProcs` (macOS `osascript`; Windows `tasklist`/`taskkill`; Linux `pgrep`/`killall`) as the only production caller, gated behind `--no-restart` (new flag on `agents install`/`agents remove`) and `--dry-run` (prints "would close/reopen `<App>`", makes no calls). A quit/reopen failure warns and never fails the command. Removed the now-dead `setup.restart_prompt_timeout_seconds` config key and the old prompt/spinner machinery in `restart.rs` (no longer referenced). Test isolation: every test that spawns the real binary for `agents install|remove` now passes `--no-restart` (centralized in `tests/common/agents.rs::raw` for the shared helper, plus the two per-plugin local helpers in `cursor_plugin.rs`/`pi_plugin.rs`, plus one direct `Command` call in `agents_install.rs`) so no test can shell out to a real `osascript`/`pgrep`/`tasklist`/`taskkill`/`killall`/`open`; `RealProcs` is constructed nowhere except inside the production `restart::run` entry point, never in the `restart.rs` unit tests, which use a `FakeProcs` recorder instead.
Check: unit tests cover running+changed (quit→write→open in order), running+unchanged (nothing), not running (write only), `--dry-run` (message, no calls), `--no-restart` (write only, no calls), quit failure (still writes, warns, no reopen), and a CLI-only host (message, never killed); a table-driven test resolves every registered host's desktop variant to its expected app/process name and panics if a new host has no row; `just check` green.
Complexity: 4/5 — new process-control trait across three platforms, an orchestration seam threaded through existing dry-run infrastructure, and an audit of every existing integration test to keep them hermetic.
Status: done 2026-09-22
...
Do: added the repo-root `.claude-plugin/marketplace.json` (name `rtok`, owner `listepo`, one plugin `rtok` with `"source": "./plugins/claude"`, verified against the Claude Code plugin-marketplaces docs — a relative `source` resolves against the marketplace root); kept the nested `plugins/claude/.claude-plugin/marketplace.json` for local/dev use. Rewrote `plugin()` in `src/agents/claude/mod.rs`: `marketplace_known(cfg)` reads `plugins/known_marketplaces.json` under `config_dir()` to skip `marketplace add` when `rtok` is already registered; steps run through `claude` only when it is on `PATH` (`super::find_on_path`), otherwise the offer fails open with "claude failed: …"; already-installed is `NO_CHANGES`; `--dry-run` only prints. `support()` for the plugin module is now `Support::Yes` (was `Support::Flag("--yes")`). Updated `src/agents/claude/README.md` and `plugins/claude/README.md` install text and `## Docs` links; regenerated `docs/agents.md` (`RTOK_BLESS=1`) — the only cell that changed is the claude CLI row's Plugin column, `` `--yes` `` → `yes`.

Ripple fixes across the test suite once the fake-`claude`-on-`PATH` test harness started exercising the new default path: `tests/claude_plugin.rs` rewritten for the no-`--yes` GitHub flow; `tests/common/agents.rs` gained `raw_without_claude()` / `rtok_without_claude()` to let tests simulate a machine without the Claude CLI; `tests/agent_remove.rs` (`claude_remove_strips_hooks_mcp_and_proxy_and_keeps_foreign`, `setup_copies_the_config_before_it_writes`) switched to the new `_without_claude` helpers since they specifically exercise the settings-file path, which now only runs when `claude` is absent; `tests/agents_install.rs`'s pipe test renamed and rewritten to assert the plugin installs silently on a pipe (no question, no declined state); `tests/trycmd/doctor.toml` and `tests/trycmd/report-md.toml` dropped the `(--yes)` suffix from the claude (cli) plugin row.

Check: unit tests for the decision logic (installed → no-op; marketplace known → only the install step; a clean install → both steps; dry-run text) without spawning a real `claude`; `readme_tables_match_support`; `agents_doc` blessed; `host_docs` green; `just check`.

Check result (2026-09-21): `cargo test` green across `agents::claude::` (17/17, including the four new decision-logic units), `tests/claude_plugin.rs` (2/2), `tests/agent_remove.rs` (14/14), `tests/agents_install.rs` (10/10), `tests/agents_real_config.rs` (9/9, 1 pre-existing `#[ignore]`), `tests/cli_trycmd.rs`, `tests/host_docs.rs` and `tests/agents_doc.rs` (bless diff was exactly the one expected cell). `mise exec -- just test` full run green.
Model: Claude Code / claude-sonnet-5
...
Execution plan: creator chose to retire `[setup].restart_prompt_timeout_seconds` (the prompt waits for an answer; an old file with the key now fails `deny_unknown_fields`, noted in `docs/config.md`). Add `inquire` to `Cargo.toml`; rewrite `src/agents/restart.rs` `offer_host_restart` onto `inquire::Confirm` (default No; Esc/Ctrl-C → No); drop the spinner, `prompt_with_spinner*`, `read_line_async`, `parse_yes_no` and their tests; remove the key from `src/config/mod.rs`, `config/default.toml`, `docs/config.md` and the three trycmd snapshots; `toolchain.md` swaps `dialoguer` for `inquire`. Verify with `just check`.
...
Do: `inquire` 0.9 (crossterm backend only, the TUI's copy) is the prompt layer of the `rtok` binary. `offer_host_restart` asks through `inquire::Confirm` (default No; Esc / Ctrl-C → No; a vanished TTY → skipped) with `RenderConfig::default()` on a colour terminal and `RenderConfig::empty()` under owo-colors' `NO_COLOR` / `TERM=dumb` rule. The creator chose to retire `[setup].restart_prompt_timeout_seconds` (inquire has no timeout; a thread-and-abandon wrapper would leave a reader stealing the next answer): removed from `Config`, `config/default.toml`, `docs/config.md` (now under Legacy keys) and the three trycmd snapshots. `prompt_with_spinner*`, `read_line_async`, `parse_yes_no`, the spinner frames and their tests are gone. `toolchain.md` and workspace `rust.md` swap `dialoguer` for `inquire`. The SDK's `accepted` stays dependency-free.
Check: unit tests in `agents::restart` pin the inquire surface — `restart_confirm_asks_about_the_host_and_defaults_to_no` (question text, default No, `y/N` hint), `restart_confirm_carries_the_colour_choice` (default vs empty `RenderConfig`), `piped_stderr_renders_plain`, `non_interactive_stdin_skips_prompt`, `confirm_answers_map_to_choices` (Esc / Ctrl-C → No, `NotTTY` → skipped, other errors surface); piped runs never prompt (T81's `an_unanswered_offer_on_a_pipe_declines_itself_and_still_installs` stays green); `just check`. A real keypress-through-inquire test needs a PTY dev-dependency — not added.
Status: done 2026-09-21
...
### T142. `rtok mcp --call` orphaned the LSP child

Bug: `src/mcp.rs`'s `run()` (the stdio `rtok mcp` server) calls `plugins::graph::lsp::shutdown()` at stdin EOF, but `call()` — the one-shot `rtok mcp --call <tool> --json <args>` used by hosts that shim MCP through a plain CLI call (the pi host, T70.3) — never did. The LSP session lives in a `static SESSION` in `src/plugins/graph/lsp.rs` that is never dropped on its own, so with `graph.backend = "lsp"` every `--call` of a graph tool (`symbol`/`callers`/`impact`/`outline`/`explore`) left rust-analyzer/clangd/typescript-language-server/dart running as an orphan after `rtok` exited.

Fix: an RAII `LspGuard` (`#[cfg(feature = "graph")]`, mirroring the `run()` gate) declared as the first statement of `call()`; its `Drop` calls `lsp::shutdown()`, so it fires on every exit path — success, `Err`, or an early `?` — the same helper `run()` already used, no new logic.

Tests: `tests/common/fake_lsp.rs` (unix-only) — a `clangd` script first on `PATH` that answers `--version` and, in server mode, speaks minimal LSP over stdio (Content-Length framing), replying immediately to `initialize` and `workspace/symbol` so `lsp.rs`'s 40 s `READY` poll never actually waits, and to a `BoomTrigger` query with a JSON-RPC error to exercise the error path. Liveness never touches a real pid: the fake takes an exclusive `flock` on a lock file in the test's temp dir and holds it for its whole life; a test proves the fake is gone by trying to acquire that same lock itself (`rtok_sys::try_lock_exclusive`, the same `flock(2)` primitive already used elsewhere in this codebase for file locking) — the OS releases the lock the instant the fake's file descriptors are torn down, for any reason. The fake exits on an explicit LSP `exit` notification immediately, on plain stdin EOF after a short grace period (long enough that a missing shutdown still shows up as "still alive" within the tests' ~2 s check), and unconditionally after a ~15 s hard self-timeout, so a failing test can never leave a process running. `tests/mcp_call_lsp_orphan.rs`: `call_shuts_down_lsp_after_a_successful_tool_call`, `call_shuts_down_lsp_after_a_tool_call_that_errors` (the fake LSP has already started and initialized when the call fails — exactly the path the old code missed), and `run_stdio_server_shuts_down_lsp_at_stdin_eof` as a regression guard for `run()`'s existing shutdown-at-EOF path. All three spawn the real `rtok` binary with an isolated `RTOK_HOME` (`graph.backend = "lsp"`), a workspace with `compile_commands.json` and no `Cargo.toml` (so `lsp::pick` chooses clangd), and `PATH` prefixed with the fake; after `rtok` exits they poll the lock file for up to 2 s. Before the fix, `call_shuts_down_lsp_after_a_successful_tool_call` failed with `fake LSP server still held its lock 2s after \`rtok mcp --call\` exited` (verified by re-running the test against a build with the `LspGuard` line removed).

Check: `cargo nextest run --test mcp_call_lsp_orphan` 3/3; `just check` green.

Complexity: 2/5.

### T143. Stop/SessionEnd hooks pile up `rtok otel flush` processes

Bug: `src/hooks/mod.rs`'s `dispatch` calls `otel::export::spawn_child` on every `Stop`/`SessionEnd` hook when OTel is configured. `spawn_child` spawns a detached, never-waited `<exe> otel flush`, and `flush`'s exclusive `flock` (`<db>.otel-flush.lock`) blocks for the whole flush. With a slow or hanging collector, a burst of `Stop` events queued one blocked `rtok otel flush` process per event, unbounded. Checked the HTTP side too: `flush_into`'s `reqwest::Client` already sets `.timeout(flush_secs)` (documented in `config/default.toml` as "proxy / mcp flush interval, and the POST timeout"), so a hung collector was already bounded — confirmed with a new unit test against a listener that accepts and never answers; no timeout fix was needed.

Fix (coalescing, at most one running + one queued hook-spawned flush): a second lock file `<db>.otel-flush-queued.lock`. `spawn_child` now does a cheap non-blocking pre-check (`queued_lock_held`) and skips spawning entirely if another hook-spawned flush is already queued — it will post whatever this event wrote. The spawned child runs `rtok otel flush --coalesce` (a hidden clap flag), which claims the queued slot non-blocking (`try_queue`); if already claimed it exits at once (`Report::default()`); otherwise it waits for the (blocking) flush lock, releases the queued slot as soon as it holds the flush lock (so a new hook-spawned flush can start queuing behind it), then flushes normally (`flush_coalesced` / `flush_coalesced_blocking` in `src/otel/export.rs`, sharing `run_flush` with the existing `flush`). A manual `rtok otel flush` (no `--coalesce`) never consults the queued lock and always flushes, as before.

Check: `tests/otel.rs::hook_spawned_flushes_coalesce_to_at_most_two_processes` reproduces the pile-up against a delayed mock collector — fires 20 real `rtok hook Stop` subprocesses and counts concurrently alive flush children via a trace directory each one marks with `<pid>.run` for its own lifetime (`RTOK_OTEL_FLUSH_TRACE`, read by `FlushTrace` in `src/otel/export.rs`; test-only, no host process table query — no `ps`, no `kill`, ever); failed on unfixed code with "at most 2 hook-spawned flush processes may be alive at once, saw 20", passes after the fix (max 2), and confirms every seeded row posts exactly once (no loss, no duplicates), waiting bounded for the trace dir to empty on its own rather than killing anything. `manual_flush_ignores_the_queued_slot` and `hook_skips_spawning_when_a_flush_is_already_queued` (warm-up call absorbs the freshly linked binary's one-time first-exec cost, matching `hooks_stay_fast_with_an_unreachable_endpoint`'s load-aware approach; asserts the trace dir stays empty) cover the two documented behaviours. Unit tests in `src/otel/export.rs` cover the queued-lock primitive (first attempt wins, others back off, frees on drop, independent of the flush lock, lock file created next to the DB) and the hung-collector timeout. `hook_spawned_flushes_coalesce_to_at_most_two_processes` isolated in `.config/nextest.toml` (`threads-required = "num-test-threads"`) since a loaded box can genuinely push concurrency past the bound, not just skew a measurement. `just check` green: fmt, clippy (workspace + wasm-demo-guest), full nextest (bench overrides untouched). `--coalesce` is `hide = true` but still shows in the AOT shell completions (bash/fish/zsh; PowerShell unaffected) — a pre-existing `clap_complete` 4.6.11 limitation, not something this change introduced: its AOT generators (`src/aot/shells/{bash,fish,zsh}.rs`) only filter hidden `PossibleValue`s, never hidden args or subcommands, so already-hidden commands like `demon upgrade` show the same way in the same snapshots. `cli_trycmd` left blessed with `--coalesce` visible for that reason.
...
Status: done 2026-09-22
Check result: `cargo nextest run --test worktree --test cli_trycmd` 4 passed (real-git fixture: tagged cache vs untagged ignored directory, dirty worktree, orphan, run from a linked worktree, not-a-repository error; `--help` goldens and completions re-blessed); `cargo nextest run --lib -E 'test(worktree)'` passed incl. the usage walk and the table snapshot; `cargo fmt --check` and `cargo clippy -p rtok --lib --bins --test worktree --test cli_trycmd -- -D warnings` clean; the full `just check` ran in CI (host disk under 20 GiB free). On this repository on 2026-09-22 the command listed 32 worktrees, 619.3 MB source against 38.2 GB tagged build cache, and the `graph-perf` orphan.
Model: Claude Code / claude-fable-5-1
...
Status: done 2026-09-22
Check result: `cargo nextest run --lib --test surface_parity --test config_coverage --test cli_trycmd --test worktree -E 'test(/worktree::/) | not binary(rtok)'` 45 passed — the verdict table (14 cases) and the real-git fixture: merged+clean+idle removed with its branch (unlocked, locked by `--owner`, and squash-merged with the base later editing the adjacent line, which only the patch-equivalence signal sees); dirty, unmerged, foreign-locked, locked-without-reason and non-idle survive with their reason; a hand-deleted directory is dropped when unlocked or locked by `--owner` and survives a foreign lock; `.git/worktrees/` afterwards holds only entries with a directory or a lock gc may not open; the dry run changes nothing; the main checkout and the worktree the command runs from are never selected. `cargo fmt --check` and scoped `cargo clippy -- -D warnings` clean; the full `just check` ran in CI (host disk under 4 GiB free). A dry run on this repository on 2026-09-22 proposed 2 of 32 worktrees and changed nothing; `--yes` was not run against the creator's worktrees.
Deviations: `--idle` defaults to `24h` (T152 reuses the flag and default); a worktree with no commits of its own reads as merged, so the idle window is what protects a freshly created one. The diff exceeds the 200-line guide: 184 lines are the fixture test and ~140 are regenerated completion goldens.
Model: Claude Code / claude-fable-5-1
...
Do (Claude Code / claude-opus-5): `spawn_child` in `src/otel/export.rs` writes `<child pid>.spawned` into `RTOK_OTEL_FLUSH_TRACE` after a successful spawn; `FlushTrace` renames `<pid>.run` to `<pid>.done` on exit. `tests/otel.rs`: `trace_files(dir, ext)` counts one kind of marker, `trace_drained` waits until nothing runs and every spawned child reported done; both T143 tests drain through it, and the skip assertion counts `.spawned` (exactly the warm-up's one) right after the hook exits, plus no `.run` left.

Status: done 2026-09-22
Check result: `cargo nextest run --test otel` 15 passed, 10 consecutive runs green; with a 1.5 s sleep injected before the child's marker the new test still passes while the old test fails; with `queued_lock_held` removed from `spawn_child` the new test fails with "queued lock held: no flush process should have been spawned at all" (left 2, right 1); `just check` green.
Model: Claude Code / claude-opus-5
...
Do (Claude Code / claude-fable-5-1): `WorktreeCmd::Add { task, slug, owner }`. `src/worktree/add.rs` — `plan(main, root_override, temp, (task, slug), owner, today) -> Result<Plan { path, branch, reason }>` is pure over paths and strings (validation, root discovery, the temp-directory refusal, the lock reason built so that `Owner::parse` reads it back) and is tested as a table; the runner resolves the main checkout from `git::list`, then `git fetch origin <default>` and one `git worktree add --lock --reason … --no-track -b …` through new `git.rs` calls (`fetch`, `add_locked`). A failed fetch is an error, not a fallback to a stale base. `[worktree] root = ""` lands in `config/default.toml`, `Config` (tilde-expanded in `finish`), `docs/config.md`; `--owner` is a per-call flag (`ALLOW_KEYS`). The T154 ledger row is left to T154 — the table does not exist yet. Tests use `testutil::tmp_dir` (`assert_fs` is not a dependency of this repo), so the temp-directory refusal is tested on `plan` with injected paths and the fixture passes an explicit root through a `--config` file.
...
Do (Claude Code / claude-fable-5-1): `WorktreeCmd::Clean { paths, idle, yes, json }`. T151's `usage` walk gains the list of tagged cache roots it already recognises (`Usage::caches`: path, bytes, newest mtime), so `list` and `clean` cannot disagree about what a cache is. `src/worktree/clean.rs` — `kept_because(cache, current, explicit, policy)` is pure and tested as a table; `run` walks every worktree git lists plus T151's orphans, or exactly the paths named (each must be one of those), and deletes one cache root at a time with `remove_dir_all`, reporting every root with its verdict so a dry run and a real run read the same; `--idle` reuses T153's flag, parser and `24h` default (per-call, `ALLOW_KEYS`, no config key). The note column shared with `gc` moves into one `worktree::noted_table` helper. Sizes and ages are not snapshot material: `trycmd` covers `--help`, the fixture asserts on `--json` and the table's summary line.

Status: done 2026-09-22
Check result: `cargo nextest run --lib --test worktree --test cli_trycmd --test surface_parity --test config_coverage -E 'test(/worktree::/) | not binary(rtok)'` 64 passed — `clean::tests` (six-case verdict table: idle goes, idle-and-named goes, never-written goes, active stays, the current worktree stays unless named; the table sums only what was or would be deleted and ends in `nothing to clean` / `dry run: … rerun with --yes` / `freed …`), `list::tests` now pins the cache root list, and the real-git fixture `clean_deletes_idle_tagged_caches_and_nothing_else`: the dry run reports 2 caches with their bytes and deletes nothing; `--yes` removes the idle worktree's and the orphan's tagged `target/`, leaves the fresh cache, the untagged `out/`, every source file and the main checkout's own cache in place; naming `.` cleans the current worktree, naming `..` is refused as "not a worktree of this repository". `--help` golden and completions re-blessed; `cargo fmt --check` and `cargo clippy --lib --bins --tests -- -D warnings` clean; the full `just check` runs in CI.
Deviations: the "output snapshot via `trycmd`" is the `--help` golden plus the fixture's assertions on the summary line — paths, sizes and ages are machine-specific. `--idle` keeps T153's `24h` default and stays a per-call flag, so `docs/config.md` gains no key.
Model: Claude Code / claude-fable-5-1
...
Do (Claude Code / claude-fable-5-1): `memory::project_name(cwd)` becomes a thin call to `project_name_with(&HostFs, cwd)` over the existing `read::fs::ReadFs` (T56.5), so the same code runs on `Vfs` in tests: walk up to the first ancestor whose `.git` reads as a file (`gitdir: …`, absolute or relative — `worktree.useRelativePaths`) or does not read at all but exists (a directory); for a file, follow `gitdir` to `<common>/worktrees/<id>`, read its `commondir` (relative, `../..`) and normalise lexically with the `read::normalize` helper — no `canonicalize`, no git spawn, so the hook budget holds; the main checkout is the common dir's parent. Name = the last segment of `url` under `[remote "origin"]` in `<common>/config` minus `.git` (`git@host:owner/repo.git`, `https://…/repo`, `file:///…/repo.git`), else the main checkout's basename. Callers (`mod.rs`, `sync.rs`) unchanged. `ReadFs` gains nothing: existence is `read_to_string(".git")` (Ok = file, `IsADirectory`/other error = directory) plus `canonicalize(".git").is_some()` for Vfs. No migration: a clone named after its repo resolves to the same name as before.
...
### Web UI: UI kit, viewport-fill, std-widgets controls (creator request 2026-09-22)

No T-number — a direct creator request rather than a `plan.md` card, recorded here retroactively per the `done.md` convention. Creator request 2026-09-22, in three steps as the work proceeded: the Slint WASM UI displayed badly — it did not fill the browser viewport and icons overflowed their nav buttons; then "inherit and reuse std-widgets where possible"; then "create a UI kit and use it without duplicating code and styles".

Do (Command Code / claude-fable-5, 2026-09-22): `crates/rtok-webui/ui/kit.slint` (new) holds the shared look — `Card`, `TitleText`, `SectionText`, `MetaText`, `BodyText`, `PageIcon`, `NavItem`, `ListRow` — with tokens from `theme.slint` (new `title-size`/`body-size`). `app.slint` went from ~1161 to ~830 lines and only composes (7 card panels, 6 list rows, 2 theme buttons, every text style dedup'd; one `default-font-family` remains). std-widgets reused with inheritance: `PluginEnableToggle` inherits `CheckBox`, the skills filter is a `CheckBox`, and expand is a `Button` (Slint 1.18's `CheckBox.toggled` takes no arguments — handlers use `self.checked`). Viewport fill: the wasm window sizes to `innerWidth`/`innerHeight` and tracks `resize` (`web-sys` +`EventTarget`); before this it stayed at the preferred 960×640. Icons pinned at 16px with min/preferred/max constraints (layouts ignore plain `width`); `MetricCell` label/value hug (`alignment: center`); the stats card hugs its rows (`if`-only children reported zero preferred height — bound `preferred-height` to a wrapper layout); equal-width icon buttons in the mobile bar; `Palette.color-scheme` set imperatively in `init`/`changed dark` (declarative `Global.prop:` bindings are a parse error in Slint 1.18) themes the native widgets. Pre-existing fixes: a missing brace in `tests/e2e.rs::snapshot_error_reaches_the_banner` (the webui test suite did not compile at HEAD) and `filter_expand` cfg-gated (dead_code on native).

Check result (2026-09-22): rtok-webui `cargo test` (SLINT_EMIT_DEBUG_INFO=1) 15/15 with all e2e label contracts intact (tabs, `toggle cmd`, `never invoked only`, `session a`, `skill hot`/`skill cold`); `cargo test --test surface_parity` 9/9; `just webui-check`, webui clippy `-D warnings`, `just dup`, `slint-viewer --check` green; pixel-reviewed renders at 375/390/820/834/960/1600/2560 (dark + light + error banner + all pages) — no clipping/overlap introduced; real-browser verification (Chrome, agent-browser): canvas rect == viewport exactly at 1440×900 / 375×667 / 2560×1440, empty console; wasm bundle 4,254,541 B vs the 4,500,000 B gate (`tests/web_wasm.rs`). Shipped as PR #187 (branch `webui-ui-kit`, commits `4c63936`, `6a38aa2`) with mobile/tablet/desktop screenshots in `site/static/images/`.

Deviations: (1) a pre-existing nit kept open — at 375px the detail viewport clips long lines at the horizontal scroll edge (content laid out at preferred width; same pixels before this work). (2) The `skills` nav icon temporarily reused `icons/doctor.svg` (no `skills.svg` existed) — resolved in the same-day `app.slint`/`kit.slint` rework, which adds `icons/skills.svg` and wires it into `kit.slint`'s `PageIcon` for `page-id == "skills"`. (3) No `plan.md` card — direct creator request; recorded here retroactively per the `done.md` convention.

Status: done 2026-09-22
Model: Command Code / claude-fable-5

### T169. No `~` config path is ever used as a literal relative path

Found 2026-09-22: every rtok worktree (and the main checkout) grew a literal `./~/.rtok/archive/<id>` and `./~/.rtok/rtok.db`. Hooks are not involved. The blob `b9ef7230…` (`line 1`…`line 200`, 1692 bytes) was committed to `main` by `1a40127 Prepare release`, so every checkout started with it. Tests also recreated it: the fixture belongs to `mcp::wrap::shorten_result_records_archive_mcp_above_threshold`, and `Runtime::in_memory` took `Config::default()` without `finish`, so `core.archive_dir` stayed `~/.rtok/archive` and resolved against the test's cwd (the crate root). A sibling production bug: `finish` expanded a hand list of path fields that skipped `setup.omp.extensions_path` and `setup.omp.mcp_path`, so `rtok agents install omp` looked under `./~/.omp/`. The test guarding that list walked a second hand list with the same gap.

Do (Claude Code / claude-opus-5-5, 2026-09-22): `src/config/mod.rs` — `Config::path_fields_mut` is now the only list of path keys (the omp pair added). `finish` and the new `Config::rebase_paths(dir)` both expand through it; `rebase_paths` resolves every `~` path under `dir`. `testutil::config_in` and `Runtime::in_memory` (now over `testutil::config`) rebase onto a fresh temp dir. The test-side hand list `path_leaves` is replaced by `tilde_leaves`, which walks the serialized config for any string that starts with `~`, so a new path key that `finish` misses fails the test. `tests/proxy.rs` handed `Config::default()` to `web::model::Model::new`, which opened `./~/.rtok/rtok.db`; both sites now use `testutil::config`. `Config::home_dir` expands a literal `~` in `RTOK_HOME` through `home_dir_from`: set from a JSON `env` block, where no shell expands it, `~/.rtok` used to put the whole store under `./~` in the cwd. The committed blob is removed from git, and `/~/` is in `.gitignore`.

Check result (2026-09-22): before the fix, `shorten_result_records_archive_mcp_above_threshold` recreated `./~/.rtok/archive/b9ef7230…`, the `proxy` test binary created `./~/.rtok/rtok.db`, and `RTOK_HOME='~/.rtok' rtok config path` printed `~/.rtok/config.toml`. After it, the lib suite and every integration binary leave no `./~`. Mutation check: dropping the omp pair and reverting `in_memory` makes `default_expands_every_pathbuf` and the new `unfinished_configs_never_keep_a_literal_tilde_path` fail with `setup.omp.* = ~/…`. The new `rtok_home_expands_a_literal_tilde` covers unset, absolute and `~` values. `just check`: 1480/1481 passed with no `./~` left behind. The one failure, `pi_plugin::setup_pi_yes_links_remove_unlinks` (a vitest run, 22.9 s under full-suite load), passes alone 2/2. An earlier run's six store-open failures came from disk pressure (1 GiB free) and pass on rerun.
...
Do (Command Code / claude-fable-5, 2026-09-22): closed out over three steps — #174 (merged, CI-reverted by `1a202b8`), the reland `6534b03`, and this change. `support("plugin")` is `Support::Offer("--yes")`, a new `Support` variant in `src/agents/mod.rs` beside `Flag`: install prints `offer plugins/kimi → /plugins install <resolved plugins/kimi> ketch install listepo/rtok` behind `--yes`, on dry-run and apply alike (only alongside a run that changes something), and rtok never writes `plugins/managed/` or `installed.json`. `installed()` reports `plugin` only on the `<kimi home>/plugins/managed/rtok/kimi.plugin.json` marker Kimi's own `/plugins install` leaves. D21 singleton: while the marker stands, setup strips the nine `[[hooks]]` tables and `mcpServers.rtok` from `config.toml` / `mcp.json` instead of adding them; remove leaves the managed copy alone and says `keep plugins/managed/rtok (owned by Kimi; remove with `/plugins remove rtok`)`. `Kimi Code Desktop` (`Kimi Code.app`) shares the same files in `VARIANTS`. `Offer` exists because neither earlier reading held: `Flag("--yes")` made `expected()` demand a read-back rtok can never produce (the `plugin did not read back as installed` warning on every plain `--yes` install, which is what failed `setup_twice`), and inflating `installed()` with `|| cfg.setup.yes` printed `✓ plugin installed` for a plugin that was never installed. `Offer` renders like `Flag` in the README and `docs/agents.md` tables (no bless needed) and is never expected, so no warning can fire against state Kimi owns. `src/agents/kimi/README.md`, `plugins/kimi/README.md` and `research.md` §15 name the store and the singleton.

Check result (2026-09-22): the three Check units green — `offer_names_the_plugin_path_and_ketch_hint`, `plugin_detected_strips_own_tables_and_reports_plugin` (seeded marker → second install strips the nine tables and `mcpServers.rtok`, reports `plugin`, remove prints the `/plugins remove rtok` line and keeps the copy), `remove_without_plugin_is_no_changes`; `plugin_manifest_matches_the_installer` green; `agents_doc` green with no bless; `setup_twice_takes_one_backup_and_says_already_installed` and the rest of `agents_install` green (the failing check before this change). `fmt-check`, `lint`, `build-min`, `dup`, `js` green; full `cargo nextest run --workspace --no-fail-fast` — 1247 run, 1246 passed, 1 failed, 4 skipped, the one failure being `agents_real_config windsurf_keeps_the_real_mcp_config_json` (T166, pre-existing and machine-state, reproduced on clean `main`; CI skips it).

Deviations: (1) the offer prints behind `--yes` and only alongside a run that changes something, not on every install — an always-printed offer line is a report line, and any non-`NO_CHANGES` report keeps a repeat install from reading back as `already installed`; without `--yes` the line stays out and the row `✗ plugin not installed (--yes)` names the flag. (2) The live precondition is still open and belongs to the creator (agents cannot drive Kimi's TUI): `/plugins install <repo>/plugins/kimi`, `/reload`, then the rtok MCP `tree` with no path must list the session project, not `plugins/managed/rtok/` — if it lists the plugin copy, the singleton must keep `mcp.json` and strip only the hooks (and `plugins/kimi/kimi.plugin.json` drops its `mcpServers`). This machine has no `plugins/managed/` yet; `plugins/kimi/README.md` keeps its "MCP cwd unverified on live Kimi" note until the probe runs.
...
Decision (creator, 2026-09-24): option (a), per-file aggregates cached by `(path, size, mtime)`.

Do (Claude Code / claude-opus-5-5, 2026-09-24): `src/measure/transcript_cache.rs` — `FileAgg { size, mtime_ns, tool_tokens, skills }` per transcript: `est_tokens` of every tool_result per tool name (the same numbers `stats::collect` puts in `tools`) and `Skill` invocations per `input.skill`. `scan` goes through a process-wide map (the web/TUI 30 s snapshot loop hits it) seeded from `transcripts-cache.json` beside the store and rewritten, pruned of deleted transcripts, only when something was parsed; tests never touch the file (a default `Config` points at the real `~/.rtok`). `doctor::read_share` sums the cached rows, minus sub-agent transcripts as `collect` does; `doctor::skill_invocations`, which parsed every transcript a second time, reads the same cache. Unreadable or unwritable cache = a re-parse, never an error.
...
Execution plan (2026-09-24): `calls.name` already holds `hook_event_name` (`record_call`, `src/hooks/mod.rs:210`); `tool_name` only lives inside `call_io.request_json`, and no column holds it, so pushing a `tool_name IN (...)` filter into SQL would need a migration — skipped per the guard rule, kept as a Rust-side check on the now-tiny row set instead. Added `Store::recent_hook_inputs_for_event(session, event, limit)` (`src/store/mod.rs`, Diesel `WHERE calls.name = ?`, no raw SQL), a `Ledger::recent_hook_inputs_for_event` trait method with a default (filter-in-memory) body for backward compatibility (`crates/rtok-plugin-sdk/src/host.rs`), and an override on `Runtime` (`src/plugin.rs`). `read::hook.rs::recently_edited` now calls it with `("PostToolUse", 5)` and dropped its manual `tool_calls` counter (the query's `LIMIT` is now the window). `memory::handoff::ledger` calls it with `("PreToolUse", LEDGER_SCAN_ROWS)`, `LEDGER_SCAN_ROWS` lowered from 200 to 50 (every fetched row is now relevant, where before most of 200 were other event types). Unit-test fixtures in both plugins called `record_call(surface, kind, None)`, leaving `calls.name` NULL — updated them to pass the same event name their fabricated JSON body carries (matches what real dispatch does), otherwise the new filter finds nothing. Added `hook_pre_tool_read_stays_under_budget_with_50_large_rows` to `src/plugins/read/hook.rs`: seeds 50 unrelated ~60 KB `PostToolUse` rows, asserts the query returns ≤5 rows and their total bytes stay under 500 KB (the real guard), plus a generous 500 ms debug-build wall-time smoke bound (the actual p95 < 10 ms budget is `cargo test --release latency`, T2.2). No migration, no new dependency. Verify: `cargo nextest run -E 'test(/read|handoff|memory|store::|hook/)'` (308 passed), `cargo clippy --all-targets -- -D warnings` (clean), `just check`.

Result: the new `Store::recent_hook_inputs_for_event(session, event, limit)` is a Diesel query filtered on `calls.name`, which already holds `hook_event_name`. It is exposed as `Ledger::recent_hook_inputs_for_event`, with a default body that filters in memory. The Read edit window fetches 5 `PostToolUse` bodies instead of 50 rows of any event. The handoff ledger fetches 100 `PreToolUse` rows instead of 200 of any event, the same reach. `tool_name` is only in the JSON, so that filter stays in Rust; a column would need a migration. Test fixtures now pass the event name to `record_call`, as production dispatch does. `hook_pre_tool_read_stays_under_budget_with_50_large_rows`: ≤ 5 rows, < 500 KB. Tests 309/309, `just check` green.
...
Execution plan (Claude Code / claude-sonnet-5, 2026-09-24): `src/measure/stats.rs` gains `RepeatReadsRow` (6 `ClassCounts { calls, bytes }`: `hook_absent`, `changed`, `ranged`, `subagent`, `fired`, `declined`) on `Report.repeat_reads`, feature-gated on `read` like `read_whole` (`#[cfg(feature = "read")]` fold, no-op stub otherwise). `fold_repeat_reads` walks one session's native `Read`/`Edit`/`Write`/`MultiEdit`/`Bash` tool uses in order (mirrors `fold_read_delta`'s path-history `Vec`): a repeat Read whose result carries a marker (`[archived `, `[rtok `, "use rtok read", "file changed since last read", "unchanged since ") → `fired`; else if an Edit/Write/MultiEdit/Bash touched the path since the last read → `changed`; else if `offset`/`limit` differ from the last read of that path → `ranged`; else if nothing in the session carries a marker → `hook_absent`; else → `declined`. Class `subagent` (T127) is filled separately: `Subagents` gains a `reread_calls` counter next to `reread_bytes` (`src/measure/subagents.rs`, `collect()`), and `stats::collect()` adds it into `report.repeat_reads.subagent` after `subagents::collect` runs — a sub-agent Read of a path the parent or an earlier sibling already read never appears in the parent's own `Parsed`, so it cannot be seen by `fold_repeat_reads`. `to_table()` prints a `repeat reads` line with per-class calls/bytes. Fixture test: one repeat per class across three session files (one plain, one with no markers at all for `hook_absent`, one parent+sub-agent pair for `subagent`). Step 3: run `rtok stats` over `~/.claude/projects` (read-only), write a dated `research.md` §2 subsection with the table, and fix the largest class only if it is a small, clear fix inside `src/plugins/read/**`; otherwise say why not in `research.md` and the report.

Check: `rtok stats` prints the repeat-read classes; the fixed class shrinks on a replayed transcript fixture; `just test` green.

Do (Claude Code / claude-sonnet-5, 2026-09-24): `src/measure/stats.rs` gains `RepeatReadsRow` (`Report.repeat_reads`, gated on the `read` feature) with per-class `ClassCounts { calls, bytes }` — `fired`, `changed`, `ranged`, `subagent`, `hook_absent`, `declined` — folded by `fold_repeat_reads` from each session's native `Read`/`Edit`/`Write`/`MultiEdit`/`Bash` tool uses; `subagent` instead comes from a new `Subagents::reread_calls` counter (`src/measure/subagents.rs`) merged in after `subagents::collect`, since a sub-agent's re-read of a path its parent/a sibling read never appears in its own `Parsed`. `to_table()` prints a `repeat reads` line. Fixture `repeat_reads_classify_each_kind_once` (`src/measure/stats.rs`) covers all six classes.

Review (coordinator, 2026-09-24): `bash_touches` counting any Bash naming the path as `changed` was unreliable (`cat`/`grep`/`sed -n`/`head` are reads too); narrowed to a write-signal table (`>`/`>>`, `sed -i`/`perl -i`/`-pi`, `tee`, `mv`/`cp`/`rm`/`touch`/`patch` naming the path, or a blanket writer — `git checkout`/`restore`/`stash`/`reset`/`rebase`/`merge`/`pull`/`apply`/`cherry-pick`, `cargo fmt`/`rustfmt`/`oxfmt`/`prettier --write` — regardless of whether the path is named), with `bash_touches_needs_a_write_signal_not_just_a_named_read` (6 cases) covering it. `stats.rs`'s diff trimmed from ~300 to 268 lines (incl. both tests): compacted `RepeatReadsRow::is_empty`, replaced the `Last` struct with a 4-tuple to dodge rustfmt's struct-lit-width explosion, shortened doc comments, and kept fixture-test literals short enough for `pair(...)` calls to stay one line under rustfmt.

`rtok stats --since 30d --json`, re-run after the `bash_touches` fix (same window, 367 sessions, 426 092 lines; dated table in `research.md` §2 "Repeat native Reads by class (T179, 2026-09-24)"): `subagent` 1 017/5.9 MB (41 % of calls, unaffected by the Bash-signal change), `changed` 732/2.1 MB (was 850/2.6 MB), `ranged` 524/2.0 MB (was 405/1.6 MB — most of `changed`'s drop landed here), `hook_absent` 111/53 KB, `fired` 55/6.9 KB, `declined` 44/58 KB. Same total calls/bytes as the first pass — only reclassification moved. Largest class is still `subagent` (d) — per plan, not fixed: T127's territory (cross-agent cache sharing is a design decision, not a small `read`-plugin patch). Largest non-(d) class, both before and after the fix, is still `changed` — no reordering, so the original fix decision stands: `changed`/`ranged` are expected behaviour by construction; `hook_absent` is T174's; `declined` — the one class that is a genuine same-session/path/range cache miss — is the smallest both times (44 calls, 0.6 % of bytes), too small to justify a code change. No fix made; see `research.md` for the full reasoning.

Check result: `mise exec -- cargo nextest run -E 'test(/stats|jsonl|read/)'` 188/188 passed (both new tests included); `cargo clippy --all-targets --all-features -- -D warnings` clean; `cargo fmt -p rtok --check` clean. Left `in progress` / not moved to `done.md` per instruction (creator review pending before close).

Result: `rtok stats` gains a `repeat reads` row group (feature `read`), which classifies every repeat native Read as `fired` / `changed` / `ranged` / `hook_absent` / `declined`, plus `subagent` from `Subagents::reread_calls`. For `changed`, a Bash command counts only when it carries a write signal (`bash_touches`). 30-day measurement in `research.md` §2: `subagent` 58.2 % of repeat bytes, `changed` 21.1 %, `ranged` 19.5 %, `hook_absent` 0.5 %, `declined` 0.6 %, `fired` 0.1 %. No class is both the largest and a small read-plugin fix: `subagent` is cross-agent sharing (T127/design), `changed` and `ranged` are correct misses, `hook_absent` is T174. So no fix was made. Tests 190/190, `just check` green.
...
Plan (2026-09-24): verified against https://geminicli.com/docs/ (fetched today) — config home `~/.gemini/settings.json` (`GEMINI_CLI_HOME` env overrides the root, no toml-side auto-read of it, same as Copilot's `dir`), one CLI binary `gemini` (`--version`, no separate desktop variant), hooks under `settings.json["hooks"]` keyed by Gemini's own event names (`BeforeTool`/`AfterTool`/`BeforeAgent`/`AfterAgent`/`BeforeModel`/`BeforeToolSelection`/`AfterModel`/`SessionStart`/`SessionEnd`/`Notification`/`PreCompress` — same list T118.1 already verified) with `{hooks:[{type:"command", command, timeout(ms)}]}` per event array, `mcpServers` alongside it in the same file. Scope split from T118.3 (extension tree, `gemini extensions install/link/uninstall`): T118.3's own text ("strip rtok's own hooks/MCP from any file setup would otherwise write directly") presupposes this task already writes those files directly, so T118.2 ships real `hooks`/`mcpServers` writes into `settings.json`, D21-unaware (no `plugin_installed()` check yet — T118.3 adds the extension, the check, and flips `plugin` support away from `No`).
Files: `src/agents/gemini/mod.rs` (+README) — `EVENTS` reverse map of `hooks::types::gemini_event` (6 pairs), own `command`/`is_ours`/`insert_ours`/`strip_ours` (no `matcher` field at all — Gemini's matcher matches its *own* tool-name spelling, not Claude's canonical names, so reusing `claude::insert_ours`'s matcher list would install a dead filter; every event fires unconditionally instead, mirroring how PostToolUse already does on every other host), `register_mcp`/`unregister_mcp` via `rtok_agent_sdk::register_server` (plain `{command,args}`, no `type` field, mirrors `windsurf`). `support()`: hooks/mcp `Yes` (Cli), `proxy` `No` (no documented base-URL setting; Gemini's own `HTTP_PROXY`/`HTTPS_PROXY` covers MCP transport only, not the model API — verified against `/docs/cli/enterprise/`), `plugin` `No` ("the extension tree ships in T118.3; no local plugin directory to link yet"). `plugin_surfaces() = [Hook, Mcp]`. No `skill::sync` call — `gemini` has no documented skill root (matches the existing `skill.rs` test `root("gemini", …).is_none()`). `src/agents/mod.rs`: `pub mod gemini;` + `HOSTS` + `host()`. `src/config/mod.rs`: `SetupGemini { dir: PathBuf = p("~/.gemini") }`, `gemini: SetupGemini` field on `Setup`, path added to `finish()`'s tilde-expansion list. `config/default.toml` + `docs/config.md` (verbatim mirror): `[setup.gemini]\ndir = "~/.gemini" # settings.json (hooks, mcpServers)`. `tests/common/agents.rs`: `.gemini` dir + `[setup.gemini]` line in `write_cfg`. `tests/agents_install.rs`: add `("gemini", vec![], Some(home.join(".gemini/settings.json")))` to the matrix `hosts()`. `docs/agents.md` regenerated (`RTOK_BLESS=1`). Regenerate `tests/trycmd/config-init.toml` (`TRYCMD=overwrite`, restore any `v[..]` wildcard trycmd overwrite strips per the shared-worktree note) since `config/default.toml` grew.
Verify: `mise exec -- cargo nextest run -E 'test(/agents|gemini|host|config|trycmd/)'`, clippy `-D warnings`, `just check`.

Result: new `src/agents/gemini/` host (mod.rs + README with `## Docs`), registered in `HOSTS`/`host()`, with `[setup.gemini]` keys. Detection: `~/.gemini` present. Install writes `hooks.<GeminiEvent>` entries (`BeforeTool`/`AfterTool`/`BeforeAgent`/`SessionStart`/`SessionEnd`/`PreCompress` → `rtok hook <ClaudeEvent> --host gemini`, 5 s timeout, no `matcher`: Gemini matches its own tool names) and `mcpServers.rtok` into `~/.gemini/settings.json`. The write is idempotent and removal keeps foreign entries. D21: no extension yet, so `support("plugin")` is No; T118.3 adds the extension and strips these entries. `docs/agents.md` re-blessed; trycmd config goldens regenerated. Agents/host/config/trycmd tests 474/474, `just dup` 1.98 % (limit 2 %), `just check` green.
...
Plan (2026-09-24): Research confirms — hooks docs (https://code.claude.com/docs/en/hooks, fetched 2026-09-24) and `research.md` §17.2 (T129, 2026-09-22): `PreToolUse`/`PostToolUse` carry `agent_id`/`agent_type` on sub-agent tool calls, no env var reaches the tool's own subprocess. MCP has no per-request signal (`src/mcp.rs` builds every `Ctx::new`, never `with_agent`; `research.md` §17.3 item 1: "MCP cannot see the caller") — confirmed by code, not just docs. Of `identical_result`'s two callers: `read/mod.rs`'s dedup check runs only inside the MCP `read` tool (`read_with` is called solely from `mcp.rs:418`; the hook's own `read::hook::pre_tool` only gates, never reads) — no signal, so Step 3 was put to the creator. **Creator decision (2026-09-24): keep today's session-only scoping on MCP `read`** — recorded here and as a dated note in `research.md` §17.3 item 1; known gap: a body one sub-agent read through MCP `read` can still reach a different context sharing that session as a pointer it never saw. `cmd::run`'s dedup runs in the `rtok run` subprocess spawned by the `PreToolUse(Bash)` rewrite (`src/plugins/cmd/hook.rs`) — that rewrite *does* see `cx.agent_id()` at dispatch time, so it now embeds `--agent <id>` (validated `[A-Za-z0-9_-]{1,64}`, else dropped — fail open, and the token sits before `--` so it never touches the POSIX/PowerShell quoting that protects the wrapped command) into the rewritten argv; `rtok run --agent <id>` threads it through `emit_filtered`/`identical_result`/`put_archive_for`. Storage: `archive` had no session+context lookup column; a session already isolates the row (`archive_in_session_hits_only_the_writer_session`, first-writer-wins on the content-addressed `id`), so a nullable `archive.agent_id` column (migration `0019_archive_agent_context`, `schema.rs`) plus filtering `archive_in_session` on `(session, agent_id)` — `IS NULL` when context is `None` — reuses that exact precedent. `Archive::put_archive`/`archive_in_session` in `rtok-plugin-sdk` keep `put_archive`'s existing signature (10+ callers; new additive default `put_archive_for(body, context)` delegates to it) and change `archive_in_session`'s signature directly (single call site, `MemoryHost` does not override it). Landed in this PR — 11 hand-written implementation/test files: `migrations/0019_archive_agent_context/up.sql`, `src/store/schema.rs`, `src/store/mod.rs`, `crates/rtok-plugin-sdk/src/host.rs`, `src/plugin.rs`, `src/plugins/cmd/run.rs`, `src/plugins/cmd/hook.rs`, `src/plugins/read/mod.rs`, `src/cli.rs`, `tests/config_coverage.rs`, `tests/lossless_roundtrip.rs` — plus `plan.md`/`research.md` (this decision) and 5 regenerated goldens (`tests/trycmd/help-subcommands.trycmd`, `tests/trycmd/completions-{bash,fish,powershell,zsh}.stdout`, via `TRYCMD=overwrite`; `man.stdout` untouched — the man page does not render per-flag detail). MCP `read` still passes `context = None` (unchanged behavior, per the creator's decision above). Verify: `cargo nextest run -E 'test(/dedup|identical|archive|hook|mcp|store::|cmd|run|trycmd/)'` — 488 passed, 0 failed; clippy `-D warnings` clean; `cargo fmt --check` clean; `just check` green.

Result:
- Migration 0019 adds a nullable `archive.agent_id` (NULL = main window, first writer wins).
- `archive_in_session` and `identical_result` take the context; the SDK gains `Archive::put_archive_for`, whose default delegates to `put_archive`.
- The PreToolUse(Bash) rewrite passes `rtok run --agent <id>` when the hook payload carries a valid `agent_id` (`[A-Za-z0-9_-]{1,64}`, else dropped, fail open). `run`/`emit_filtered` archive and dedup under it.
- MCP `read` passes `None`. Creator decision 2026-09-24: keep session-only scoping there; known gap recorded in `research.md` §17.3.

Tests: `dedup_is_scoped_to_the_dispatching_agent` (A archives, B gets the body, A gets the pointer), `archive_in_session_scopes_by_context_within_one_session`, `identical_result_is_scoped_to_the_writer_context`, and hook tests for the flag. Tests 520/520, `just check` green.
...
Do (2026-09-24): T230's pattern, copied, with one difference — `agents list`/`agents info` were already `--json`-gated (T60.1 done), so no CLI change was needed there. `Snapshot.hosts: String` (never fails, unlike `graph`/`stats` — `agents list` cannot error) is `hosts_page_text`, which wraps `crate::agents::list(cfg)` verbatim (the same blocks `agents list` prints: kind, name, app path+version, config files, module/plugin lines — no reformatting, D27 one probe two renderings). `agents list` spawns one `--version` per host variant (T168), too slow for a 2 s tick, so `hosts_page_text` reuses `doctor_for_snapshot`'s cache shape (`DOCTOR_SNAPSHOT_TTL`) but never blocks on a cold/stale entry: a background thread (guarded by an `AtomicBool` so only one refresh is in flight) refreshes the cache while the tick renders the last known text, or "probing hosts…\n" before the first probe lands. `model::pages()` gains `("hosts", "hosts")`. TUI: `view::hosts_page` renders `snapshot().hosts` verbatim (`"hosts" =>` arm); the 10th tab no longer fit the 90-col test terminal (T230 left headroom for one more, not two) — widened to 98, documented in the comment. Web: Slint gains a `hosts` page (`PAGE_IDS`, `hosts-text` property, `hosts_of`/`set_hosts_text`, no new icon). `agents list`/`agents info` move from `EXEMPT` to `COMMAND_PAGES` (mapped to `hosts`); removed from the "writing" `EXEMPT` block, which was wrong for them (they read, they do not mutate a host) — the two entries are gone, the comment now only covers what it says. Tests: `hosts_page_exists_on_both_surfaces`, `tests/hosts_model.rs`'s `hosts_page_matches_agents_list_json_on_the_fixture_matrix` — spawns real `rtok web` + `rtok agents list --json` subprocesses on `tests/common/agents.rs`'s fixture matrix (`write_cfg` + `fake_claude_path`, both on the identical PATH/HOME so app/version fields agree without needing process-env mutation, which this crate forbids), polls `/ws` snapshots past the "probing hosts…" cold state, asserts every JSON row's block/config path is in the page text. Files: `src/web/model.rs`, `src/tui/view.rs`, `crates/rtok-webui/src/lib.rs`, `crates/rtok-webui/ui/app.slint`, `tests/surface_parity.rs`, `tests/hosts_model.rs`, `plan.md`, `todo.md`.

Result: page `("hosts","hosts")` on tui and web renders `agents::list(cfg)` verbatim, the same probe as `agents list --json`. It is cached with the doctor-snapshot TTL; a cold or stale entry is refreshed on a background thread (AtomicBool-guarded) while the tick shows the last text or "probing hosts…", so a tick never waits on host `--version` probes. `agents list`/`agents info` moved to `COMMAND_PAGES`; the "writing commands" comment was fixed. The TUI test terminal is 98 columns for 10 tabs. Test: `hosts_page_matches_agents_list_json_on_the_fixture_matrix` spawns `rtok web` + `agents list --json` on the fixture matrix (60 s deadlines for CI). Scoped suite 195/195, webui lib 7/7, `just check` green.
...
Plan (2026-09-24): verified against Hmbown/Codewhale `docs/HOOKS.md`, `docs/MCP.md`, `docs/CONFIGURATION.md` (raw.githubusercontent.com, fetched today). Config home `$CODEWHALE_HOME` (default `~/.codewhale`, legacy `~/.deepseek`); `config.toml` carries `[hooks] enabled` + `[[hooks.hooks]]` (`event`, `command`, `name`, `timeout_secs`), sibling `mcp.json` carries `mcpServers.<name> = {command, args, env}` (`codewhale mcp add`/`mcp init` are the documented CLI path; every other rtok host writes the file directly instead, so this one does too). CodeWhale's 15 hook events split observer/steering; only 3 are steering (`message_submit`, `tool_call_before`, `shell_env`) and only 7 more carry any stdin JSON at all (`turn_end`, `subagent_spawn`, `subagent_complete`, `session_busy`, `session_idle`, `session_error`, `waiting_for_user`) — HOOKS.md states this in so many words ("The remaining observer events — `session_start`, `session_end`, `tool_call_after`, `mode_change`, `on_error` — receive environment variables only, with no stdin payload"). `tool_call_before` (the PreToolUse analogue) is env-var-only too — no stdin JSON — so honoring it would mean changing the shared stdin-JSON entry point every host relies on (`src/hooks/mod.rs::dispatch_owned_strict` parses stdin unconditionally; empty stdin already degrades to the safe no-op `{}` today, D1), not a small per-host adapter — left unwired, documented in the host README. The 7 JSON-bearing observer events are pure observers Codewhale never reads a reply from ("Codewhale ignores the hook's **result**"), and their fields (`agent_id`, `from`/`to`, …) do not match rtok's `SubagentStart`/`SessionStart` shapes — left unwired too. Only `message_submit` is both steering (rewrite-capable) and stdin-JSON-real: wired to `UserPromptSubmit` via a small `--host codewhale` adapter (`src/hooks/types.rs::adapt_codewhale`, `src/hooks/mod.rs::codewhale_output`) mirroring T118.1's Gemini adapter — `text` → `prompt` in, `hookSpecificOutput.additionalContext` folded into a full `{"text": "..."}` replacement out (CodeWhale has no separate "add context" field for this event). `proxy`: `[providers.<name>].base_url` tables, no single override (Kimi's reasoning applies verbatim). `plugin`: CodeWhale's plugin-bundle system needs a reviewed remote source and exact environment-source references (`docs/PLUGIN_BUNDLES.md`) — no local directory to link, so v1 ships no plugin bundle (step 4).
Files: `src/agents/codewhale/mod.rs` (+README, new) — one CLI variant (`codewhale`/`codew` on PATH), `[hooks]`/`[[hooks.hooks]]` via `toml_edit` (mirrors `kimi`'s pattern, one nesting level deeper), `mcpServers.rtok` via `rtok_agent_sdk::register_mcp`/`unregister_mcp`. `src/agents/mod.rs`: `pub mod codewhale;` + `HOSTS` + `host()`, plus two shared helpers extracted to hold the dup gate under 2.00% once a 16th host's boilerplate pushed it to 2.08% — `load_toml` (an absent-file-is-empty-document TOML read, previously copy-pasted in `codex`/`kimi`/`aider`) and `apply_hook_and_mcp` (the "write the hook, then register/unregister MCP" `Agent::apply` body, previously copy-pasted in `gemini`); both are additive, existing hosts untouched. `src/config/mod.rs`: `SetupCodewhale { dir: PathBuf = p("~/.codewhale") }`, `codewhale: SetupCodewhale` on `Setup`, path added to `path_fields_mut()`'s tilde-expansion list. `config/default.toml` + `docs/config.md`: `[setup.codewhale]\ndir = "~/.codewhale"`. `src/hooks/types.rs::adapt_codewhale`, `src/hooks/mod.rs::codewhale_output` + `dispatch_owned_strict` branch. `tests/common/agents.rs`: `.codewhale` dir + config line in `write_cfg`. `tests/agents_install.rs`: `("codewhale", vec!["--yes"], Some(home.join(".codewhale/config.toml")))` in `hosts()`. Generated (excluded from the file/LOC budget): `docs/agents.md` (`RTOK_BLESS=1`), `tests/trycmd/config-init.toml`, `config-show.stdout`, `report-md.toml` (`TRYCMD=overwrite`; `man.stdout` untouched, no `v[..]` wildcard to restore).
Verify (2026-09-24, all green): `cargo nextest run -E 'test(/agents|codewhale|host|config|trycmd|hook/)'` — 548/548 passed; `cargo clippy --all-targets -- -D warnings` clean; `cargo fmt --check` clean; `just dup` 1.99% (170 clones, 0 involving `codewhale/mod.rs`); `just check` — 1611/1611 tests passed (3 skipped, local-only), oxlint/oxfmt/pytest clean. Hand-written: 10 files (`src/agents/codewhale/{mod.rs,README.md}`, `src/agents/mod.rs`, `src/config/mod.rs`, `config/default.toml`, `docs/config.md`, `src/hooks/{mod.rs,types.rs}`, `tests/agents_install.rs`, `tests/common/agents.rs`), ~541 LOC by `git diff --stat`/`wc -l` (over the ≤200 aim — `mod.rs` alone is 333 lines, ~115 of them tests; in line with T118.2's Gemini precedent at 291 lines for one file). Only `message_submit`→`UserPromptSubmit` is wired; `tool_call_before` and the 7 stdin-JSON observer events are documented as left out in the README, not silently dropped.

Result: `rtok agents install codewhale` writes one `[[hooks.hooks]]` entry (`message_submit` → `rtok hook UserPromptSubmit --host codewhale`) into `$CODEWHALE_HOME/config.toml` and `mcpServers.rtok` into `mcp.json`; `adapt_codewhale` maps `text` to `prompt`, `codewhale_output` folds plugin context into the `{"text"}` replacement CodeWhale reads. The other 14 events stay unwired: env-only (no stdin) or observers whose reply CodeWhale ignores. Proxy and plugin bundles are `Support::No` with reasons. `[setup.codewhale]` added; shared `load_toml`/`apply_hook_and_mcp` helpers keep `just dup` at 1.99 %. `just check` green (1611 tests).
...
Do (2026-09-24): re-listed the families at claim time via the same `settings.pick(bin(&family_argv(..)))` path `formatters::compress` uses; missing goldens were `curl`, `node`, `pnpm`, `sed`. Wrote one 46-line `.in` per family (filler lines plus one realistic error/warning line matching that family's `keep` pattern or landing in its head/tail window), generated each `.out` with a throwaway test calling `compress()` directly and writing the result to disk (removed after use — this crate has no `RTOK_BLESS`/bless env var), and read every `.out` by hand to confirm the error line survives and the output is shorter. `curl.in`'s `curl: (7) Failed to connect...` line matches `[curl]`'s `keep`; `node.in`'s `Error: Cannot find module...` matches `[node]`'s; `pnpm.in`'s ` ERR_PNPM_NO_MATCHING_VERSION ...` matches `[pnpm]`'s. The first `sed.in` draft used `sed -n 1,200p report.log`, which `bounded::is_bounded` (T176) recognizes as an already-bounded read and passes through unchanged (0% saving, by design, not a bug) — switched the argv to an unbounded `sed -i.bak s/.../.../g config/routes.conf` substitution so it actually reaches the `[sed]` TOML rule instead. Writing the required coverage test (`every_builtin_rule_family_has_a_golden`, `src/plugins/cmd/formatters.rs`: lists every `match_cmd` in `rules::defaults()` and fails naming any family with no `.in` whose argv routes to it via `bin(&family_argv(..))`, `[script]` exempted since it's reached by mixed-chain detection rather than an argv[0] match) also caught that the pre-existing `gradle.in` (`argv: ./gradlew test`) stems to `"gradlew"` (`cmd_stem` only strips path/`.exe`), never `"gradle"`, so it silently exercised the default rule instead of `[gradle]` — added `gradle_bare.in`/`.out` (`argv: gradle build`) alongside it rather than touching the existing fixture, which still separately validates the wrapper-falls-to-default path. Mutation check: moved `curl.in`/`curl.out` out of the directory, `every_builtin_rule_family_has_a_golden` failed naming exactly `["curl"]`, restored both files.
