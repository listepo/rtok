...
Deviations: (1) a pre-existing nit kept open — at 375px the detail viewport clips long lines at the horizontal scroll edge (content laid out at preferred width; same pixels before this work). (2) The `skills` nav icon temporarily reuses `icons/doctor.svg` (no `skills.svg` existed). (3) No `plan.md` card — direct creator request; recorded here retroactively per the `done.md` convention.
