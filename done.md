...
Plan: install prints the exact `/plugins install <resolved plugins/kimi path>` line (dry-run and apply alike; rtok never writes `plugins/managed/` or `installed.json`); `installed()` reports `plugin` when `<kimi home>/plugins/managed/rtok/kimi.plugin.json` exists; D21 singleton — while the plugin is installed, setup strips rtok's own `[[hooks]]` tables and `mcpServers.rtok` instead of adding them; a `Desktop` variant joins `VARIANTS`; README module table, `docs/agents.md` bless, `research.md` §15 sentence.

Do (Muse Spark / rtok): `offer_plugin()` in `src/agents/kimi/mod.rs` prints the `/plugins install` line with the resolved `plugins/kimi` path + ketch hint on every install run and a `/plugins remove rtok` keep-line on remove; `plugin_marker()` / `plugin_detected()` read the managed copy; `apply()` strips own tables + unregisters own MCP while detected (Cursor's `plugin_is_mcp` rule, for hooks too); Desktop variant (`Kimi Code.app`, same files) + `restart.rs` expected-name row; `support(plugin)`: `No` → `Yes` (the offer is always printed, never gated — a `Flag("--yes")` would make `expected()` demand `plugin` in `installed()` after a plain install, which the Kimi-owned store can never satisfy); README module table; `docs/agents.md` re-blessed; `research.md` §15 sentence; trycmd `doctor.toml` / `report-md.toml` re-blessed under a hermetic HOME. Tests: 3 new `agents::kimi` units (offer names path + ketch on dry-run and apply; singleton strip + `plugin` report; clean remove `NO_CHANGES`), `tests/kimi_plugin.rs` (manifest = 9 hooks + one `rtok mcp` server, no call-path duplication, no `scripts/`), `kimi_plugin_singleton_strips_own_tables_and_keeps_the_managed_copy` e2e in `tests/agent_remove.rs`; `plugins/kimi/README.md` singleton paragraph.

Status: done 2026-09-22
Check result: `agents::kimi` 10 passed; `kimi_plugin` 3 passed; `agent_remove` 15 passed; lib 911 passed (1 pre-existing `codex --version` hang skipped — hangs on main too); `agents_doc` / `host_docs` / `config_coverage` / `cli_trycmd` green; fmt + clippy clean. Live (kimi 0.39.1): offer/strip/remove cycle verified on an isolated HOME; precondition partially verified — stdio `tree` lists the session project cwd via user `mcp.json`; real `/plugins install` needs the interactive TUI (trust gate blocks piped stdin) and a funded wallet (TeamoRouter 400), left to the creator.
Deviations: `support(plugin)` is `Yes`, not the card's `Flag("--yes")` (reason above); the tree-cwd precondition (plugin copy vs session project) stays unverified, so the singleton strips both hooks and MCP until measured otherwise.
