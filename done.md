...
Check: `confirm_answers_map_to_choices` pins the answer mapping; piped runs never prompt (`should_prompt` unchanged; T81's `an_unanswered_offer_on_a_pipe_declines_itself_and_still_installs` stays green); `just check`.
Status: done 2026-09-21
Check result: locally `cargo fmt --check` and `cargo clippy --workspace --all-targets --all-features -D warnings` green; nextest could not link (host disk full, errno 28) — the test run is CI's on the PR.
Model: Claude Code / claude-opus-5
