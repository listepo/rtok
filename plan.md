...
**Blocker 2026-09-07 (user decision).** `dist-workspace.toml` lists `homebrew` under `installers` and `publish-jobs`, so `release.yml` pushes a formula to `listepo/homebrew-tap` with `secrets.HOMEBREW_TAP_TOKEN` — the repo does not exist and the secret is not set, so the workflow would push the version bump and then fail in `publish-homebrew-formula`. Either create the tap and the secret, or drop `homebrew` from both lists, before starting the workflow.
