...
Status: plan v1, 2026-09-01. **Progress: 183 ✅, 14 open (v0.2+ P28–P33, P35; P36 residue done; P39 closed — SQLite only); entries in `done.md`.** Companion evidence: `research.md` (comparison, measurements, fact-check). Shape of the code: `architecture.md`. Per-plugin plan: `roadmap.md`. Propositions (not yet tasks): `ideas.md`. Every implemented task must be marked done and moved from here to `done.md` verbatim (Do/Check + `Status: done <date>` and Check result); a task that still lives here is not done.
...
| D18 | **The graph index lives in SQLite with the ledgers (D8).** LadybugDB (`lbug` / `graph-lbug` / `symbols_lbug.rs`) was gated (Gate P8c, 2026-09-08): clause (4) won 77× on `impact(4)`, but (2)/(3)/(5) failed (hook p95, warm calls, cmake C++ cost) so it stayed opt-in then frozen. Grafeo (`graph-grafeo`) was spiked (T8.20 / P8e, 2026-09-12) as a pure-Rust stand-in and abandoned (warm `impact(2)` ~22 000× slower; fan-out DNF). **P39 (2026-09-12): both removed** — no live feature flags; historical numbers stay in `research.md`. D13 holds: SQL for symbols lives only in `src/store/symbols.rs`. D6 holds: no spawned graph tool. | Field tools use graph stores and `impact` is the hop-bounded query where a path pattern can beat N lookups; both candidates were priced and deleted per the gate (loser deleted). Survey archive: `src/plugins/graph/PLAN.md`. |
...
- Graph backends (2026-09-12, P39 closed): do not claim tasks that reintroduce `lbug` / `graph-lbug` / `symbols_lbug.rs` / `grafeo` / `graph-grafeo` / cmake-for-liblbug. Symbol index is SQLite only. SQLite `graph` work (P35) is fine.
...
### P8c — `graph` on LadybugDB — tasks done; Gate P8c: clause (4) won 2026-09-08; code deleted under P39 (see `done.md` P8c / P39).
...
| v0.2 | **Replace LadybugDB** — **done P39 (2026-09-12):** keep SQLite; delete `lbug` / Grafeo. | **P39** (I-30) ✅ |
...
### P39 — Replace LadybugDB — added 2026-09-12 (I-30); **closed 2026-09-12**.

**Decision: SQLite only.** Delete LadybugDB (`graph-lbug`, `lbug`, `src/store/symbols_lbug.rs`, cmake/`LBUG_BUILD_FROM_SOURCE` pin) and do not merge Grafeo (`graph-grafeo`, `symbols_grafeo.rs`). Grafeo was measured on the abandon spike (PR #22 / T8.20) and lost Gate P8c-style bars hard; Ladybug remains archived as the P8c opt-in that never cleared hook/warm/build cost. No live alternate graph feature flags.

**T39.0 survey: LadybugDB replacement** · — · `src/plugins/graph/PLAN.md`, `research.md`, code removal
Do: name the winner. Survey options included keep-SQLite-and-delete-lbug, Grafeo (pure-Rust LPG), and leave frozen lbug. Winner = SQLite; remove both backends and their deps; archive numbers in `research.md`.
Check: PLAN.md + `research.md` record the decision; `Cargo.toml` has no `graph-lbug` / `graph-grafeo`; `just check` green on default features.
Complexity: 3/5
Status: done 2026-09-12
Model: -

Gate P39 (review): **passed** — SQLite-only; losers deleted (D18).
...
| 2026-09-12 | P39 closed: SQLite only; remove LadybugDB (`graph-lbug`) and abandon/remove Grafeo (`graph-grafeo`). Historical P8c/P8e numbers kept in `research.md`. | Measurement + cost; D18 loser deleted. |
