...
Status: plan v1, 2026-09-01. **Progress: 182 ✅, 15 open (v0.2+ P28–P33, P35, P39 LadybugDB replacement; P36 residue done); entries in `done.md`.** Companion evidence: `research.md` (comparison, measurements, fact-check). Shape of the code: `architecture.md`. Per-plugin plan: `roadmap.md`. Propositions (not yet tasks): `ideas.md`. Every implemented task must be marked done and moved from here to `done.md` verbatim (Do/Check + `Status: done <date>` and Check result); a task that still lives here is not done.
...
- LadybugDB freeze (2026-09-12): do not claim new tasks whose Do is `lbug` / `graph-lbug` / `src/store/symbols_lbug.rs` / cmake-for-liblbug / P38-style "graph only on LadybugDB". SQLite `graph` work (T35.3) is not that freeze. Compiling the existing `graph-lbug` feature in `just check` is not a claim. The replacement survey is P39 only.
...
### P8e — Grafeo spike (opt-in `graph-grafeo`) — added 2026-09-12; done (abandon).
...
Check: `cargo test --features graph-grafeo --test graph_contract`; release `graph_bench --ignored` under default and `graph-grafeo`; `just check` still green on the default feature set. Measured: abandon (research.md).
Complexity: 4/5
Status: in progress
Model: Composer
...
Gate P15 (T15.1–T15.9): the TUI scaffold and shell are landed; the tab set is
...
| v0.2 | **Replace LadybugDB** — survey an embedded graph store that can win Gate P8c without the C++/cmake cost, or keep SQLite and delete `lbug`. No new `lbug` work until then. | **P39** (I-30) |
...
### P39 — Replace LadybugDB (goal: name an embedded graph store that can win Gate P8c without the C++/cmake cost, or keep SQLite and delete `lbug`) — added 2026-09-12 (I-30); open.

LadybugDB (`lbug`) stays the frozen opt-in `graph-lbug` backend. Do not add lbug features, index-speed work on `symbols_lbug.rs`, or a LadybugDB-only default (the graph-perf P38 line does not land on `main`). P8c clause (4) still holds on the 2026-09-08 numbers; clauses (2)(3)(5) and the cmake/`sccache` build cost are why a replacement is in scope.

**T39.0 survey: LadybugDB replacement** · — · `src/plugins/graph/PLAN.md` (extend v0.3), `research.md` as needed
Do: survey ≥3 embedded backends against D6 (library, not a spawned tool), D8 (ledgers stay in `rtok.db`; graph index may be a derived file), D13 (queries only in `src/store/`), D18 (one writer; loser deleted), D1 (hook ≤10 ms). Include at least: keep SQLite CTE/BFS and delete `lbug`; a pure-Rust persistent graph; one other. Revisit the 2026-09-04 table in `src/plugins/graph/PLAN.md` (petgraph, indradb, cozo, kuzu) with current versions. No implementation. No new dependency.
Check: PLAN.md names one winner (or "SQLite, delete lbug") with a Gate P39 bar that reuses P8c clauses; docs-only change; `just check` green.
Complexity: 3/5
Status: open
Model: -

Gate P39 (review): a named replacement (or SQLite-only) that can be implemented without claiming new `lbug` work; existing `graph-lbug` code stays until the winner's later Check deletes it (D18).
...
| `T8.20` | P8e Grafeo spike | opt-in `graph-grafeo` + P8c numbers | done (abandon) | 4/5 |
...
| `T15.5` | P15 tui | Calls tab (P13 rows + detail) *(`roadmap.md`)* | ✅ 2026-09-10 | 3/5 |
...
| `T15.9` | P15 tui | TTY guard, `q` restores the terminal *(`roadmap.md`)* | ✅ 2026-09-10 | 2/5 |
...
| 2026-09-12 | P39 added (I-30): survey a LadybugDB replacement. §2 freeze: do not claim new `lbug` / `graph-lbug` / `symbols_lbug` / P38-style tasks. D18 amended: `graph-lbug` stays frozen opt-in until P39 names a winner. T35.3 remains SQLite-only. | User request 2026-09-12: найти замену для ladybugdb; новые задачи связанные с ней не брать. |
