...
Verify first (`omp` is installed on this machine, `~/.omp/agent/` does not exist yet): (a) the linked extension loads under omp and a bash call goes through `rtok run`; (b) resolved by reading 2026-09-21: `registerPiTools` in `plugins/pi/extensions/rtok.ts` runs `rtok config get setup.pi.tools` and registers the tools when it prints `true` — it has no idea which host loaded it, so a machine with pi (`tools = true`) and omp would get every tool twice under omp (registerTool + native MCP), against D21. The extension therefore needs a host signal before it registers: find what omp exposes (a field on the `ExtensionAPI`, `process.argv` / `process.title`, or an env var) and skip `registerPiTools` under omp; a unit test in `plugins/pi/tests/` pins it. The agent's shell allowlist blocks the `omp` binary, so the creator runs the live parts of (a)–(c) or allows it (`lean-ctx allow omp`); (c) omp also imports MCP servers from Claude Code / Cursor configs — confirm that a native `rtok` entry plus an imported `rtok` entry start **one** `rtok mcp` (D21 singleton); if omp starts two, the README documents `disabledServers` and the task stops for a creator decision.
Over the ≤200 LOC / ≤3 files limit as written — split into T92.1 (host + extension link + config) and T92.2 (MCP registration + docs bless) when claiming.
