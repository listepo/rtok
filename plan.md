...
| --- | --- | --- | --- | --- | --- |
| T83.2 | in progress | P1 | 3 | 10% | Claude Code / claude-opus-5-5 |
| T83.4 | todo | P1 | 3 | 0% | |
...
Found 2026-09-25: `run.rs` does not hardcode `sh -c`; `resolve_shell` falls back to cmd.exe on Windows and T83.8 fixed how the cmd.exe body is quoted. The tests hardcode POSIX commands: `printf "a\nb\n"`, `printf a; printf b` and `sh -c "exit 3"`. cmd.exe has no `printf`, a newline ends its command line, and there is no `sh`.
Plan: in `src/plugins/cmd/run.rs` tests, when `shell_kind(&shell(&c))` is `Cmd`, print the same bytes with `type <file>` (the T83.11 pattern), chain two `type`s with `&` for the compound case, and use `exit 3` for the exit-code case; POSIX shells keep the current commands. Drop the `plugins::cmd::run` line from `.config/nextest.toml`; the PR's `windows` job is the check.
