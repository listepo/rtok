...
| --- | --- | --- | --- | --- | --- |
| T83.4 | in progress | P1 | 3 | 60% | Claude Code / claude-opus-5-5 |
...
Found 2026-09-25 (PR #374's windows job with the override removed): four causes, none in the installers. (1) `cursor_plugin` and `pi_plugin` write their own `config.toml` with `"{path}"`, and a `\` in a TOML basic string starts an escape (`invalid unicode 8-digit hex code`); the shared `write_cfg` already writes `/`. (2) `agents_install` and `opencode_plugin` look for `home.join(".codex/config.toml").display()` in output that prints the `/`-joined configured path with OS-joined children, so Windows lines mix `/` and `\`. (3) vitest fails at startup: mise's npm installer (aube) leaves `@vitest/mocker` without its `vite` peer link and the hoisted `.mise/node_modules/vite`, and the `NODE_PATH` workaround in `tests/common/mod.rs` never worked because Node's ESM resolver ignores `NODE_PATH` (reproduced on macOS by deleting both links in a copy of the install). (4) `windows_ci::windows_exclusion_list_names_only_existing_tests` requires the override that this task deletes.
Plan: (1) `/`-join those two configs like `write_cfg`. (2) new `common::agents::slash`; compare both sides through it. (3) `tests/node/vite-peer.mjs`, preloaded with `--import`, registers a `module.registerHooks` resolve hook that retries a bare `vite` from the `npm:vite` install; it replaces the `NODE_PATH` code. (4) delete that guard. Then the closing criterion: delete the `cfg(windows)` override, drop `continue-on-error` from the `windows` job and add it to `revert-on-failure`'s `needs`.
