...
Creator request 2026-09-21, after T119. The first `just codeql` run reports 16 results; the scan is only useful once it is empty. (1) `actions/unpinned-tag` ×13 in `bump.yml`, `ci.yml`, `docs.yml`, `release-plz.yml`, `release.yml`, `verify.yml`: pin every third-party action to a full commit SHA with the tag as a trailing comment (`uses: owner/action@<sha> # v4`) at the version already in use — no bumps. `release.yml` is generated by cargo-dist: pin through `dist-workspace.toml` (`github-action-commits`) and `just dist-generate`, not by hand. (2) `actions/missing-workflow-permissions` ×2 in `ci.yml`: add a top-level `permissions: contents: read`; jobs that need more (`revert-on-failure`) already declare their own. (3) `rust/log-injection` in `tests/web.rs:223`: test code — fix it if the change is trivial, otherwise suppress with a one-line reason.

Check: `just codeql` reports 0 results for all four languages; `actionlint` clean on every workflow; `just check`; the next `ci` and `codeql` runs on `main` are green.
