...
| `rtok worktree list` | every git worktree of the repository with its owner (the lock reason, else the newest session the hooks saw working there), state, source bytes and tagged build-cache bytes, plus orphans git no longer lists (`--json`); read-only |
