...
replaces bash output with `rtok filter` (fail open: any spawn error returns the original
output). `experimental.session.compacting` calls `rtok hook PreCompact --host opencode` and
