...
- `hooks/hooks.json` — `beforeShellExecution` → `rtok hook PreToolUse --host cursor`,
  `afterShellExecution` → `rtok hook PostToolUse --host cursor`, `afterMCPExecution` → `rtok hook AfterMCPExecution --host cursor`.
...
- Hooks (`"version": 1`, `beforeShellExecution`, `afterShellExecution`, `afterMCPExecution`): https://cursor.com/docs/agent/hooks
