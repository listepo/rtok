...
- `hooks/hooks.json` — `beforeShellExecution` → `rtok hook PreToolUse --host cursor`,
  ...
  `postToolUse` (matcher `MCP:`) → `rtok hook PostToolUse --host cursor` (replaces long MCP
  results via `updated_mcp_tool_output`).
...
- Hooks (`"version": 1`, `beforeShellExecution`, `afterShellExecution`, `postToolUse` / `updated_mcp_tool_output`): https://cursor.com/docs/agent/hooks
