...
- **Model routing (D9 / §16.3 #3)** — **planned / Later**, separate from Batch/Flex: cheap model for format/classify/expand-prep, mid for edit, expensive only after confirm; proxy may rewrite `model` under `[proxy.routing]` after a measurement Check (`docs/model-routing.md`). Stub config only until then.
- **LLM soft compression (P28)** — Later / default-off LLMLingua-class shrink of archived context; ship only if a bench beats v0.1 lossless (`plan.md` P28, I-21). Roadmap: [`docs/llm-soft-compression.md`](docs/llm-soft-compression.md) (extractive lane already shipped; LLM lane gated).
